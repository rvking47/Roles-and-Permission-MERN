...
import { usePermissions } from "../pages/context/PermissionContext";
...
const Sidebar = ({ isOpen, name, role, handleLogout }) => {
  ...
  const AdminisDisabled = role === "admin";
  const UserisDisabled = role === "user";
  const ManagerisDisabled = role === "manager";
  return (
    <aside
    ...
    >
      ...
      <div>
        <ul className="space-y-3 mt-5 pt-5 pr-5 nunito-uniquifier">
          ...
          {UserisDisabled && (
            <Link to="/users/user" className="text-black flex items-center hover:bg-gray-100 rounded-lg cursor-pointer" style={{ textDecoration: "none" }}>
              <li className="flex items-center gap-3 p-2">
                <FiHome className="text-lg" /> Dashboard
              </li>
            </Link>
          ...
          {ManagerisDisabled && (
            <Link to="/users/manager" className="text-black flex items-center hover:bg-gray-100 rounded-lg cursor-pointer" style={{ textDecoration: "none" }}>
              <li className="flex items-center gap-3 p-2">
                <FiHome className="text-lg" /> Dashboard
              </li>
            </Link>
          ...
          {/* Users */}
          {permissions.includes("list_user") && (
